...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <h2 style="text-align: center; color: #333;">Receipt {{ $purchase->receipt_number }}</h2>

        <p>Dear {{ $purchase->supplier->name }},</p>

        <p>Please find attached the receipt acknowledging payment for the fish purchased on {{ $purchase->purchase_date->format('F d, Y') }}.</p>
        
        <div style="background-color: #f5f5f5; padding: 15px; border-radius: 5px; margin: 20px 0;">
            <p><strong>Purchase Date:</strong> {{ $purchase->purchase_date->format('F d, Y') }}</p>
            <p><strong>Amount Paid:</strong> SBD {{ number_format($purchase->total_cost, 2) }}</p>
        </div>
